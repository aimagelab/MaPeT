...
class LayerScale(nn.Module):
    def __init__(self, dim, init_values=1e-5, inplace=False):
        ...
        # self.gamma = nn.Parameter(torch.diag(torch.Tensor(init_values * torch.ones(dim))))

    def forward(self, x):
        # return x.mul_(self.gamma) if self.inplace else torch.matmul(x,self.gamma)
        # return x * self.gamma
...
class StandardVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if pre_training_checkpoint:
            self.init_from_pretraining(pre_training_checkpoint)
    ...
    def init_from_pretraining(self, checkpoint):
        self.init_weights(mode='')
        _logger.info("Checkpoint path: %s", checkpoint)
        # print("Checkpoint path", checkpoint)
        checkpoint = torch.load(checkpoint, map_location="cpu")
        _logger.info("Pre_training epoch loaded:  %s", checkpoint["epoch"])
        pretrained_dict1 = {
            k.replace("Double", "").replace("encoder.", "").replace("blocks_", "blocks."): v for
            k, v in
            checkpoint["state_dict"].items() if
            k.replace("encoder.", "").replace("blocks_", "blocks.") in self.state_dict()}
        difference = list(set(self.state_dict()) - set(pretrained_dict1))

        print(difference)
        self.load_state_dict(pretrained_dict1, strict=False)

    def get_num_layers(self):
        return len(self.blocks)

    def get_intermediate_layers(self, x, last=False):
        x = self.patch_embed(x)
...
@torch.no_grad()
def _load_weights(model: StandardVisionTransformer, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        StandardVisionTransformer, variant, pretrained,
...
@register_model
def vit_standard_tiny_patch16_224(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, **kwargs)
    model = _create_vision_transformer('vit_standard_tiny_patch16_224', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_standard_small_patch16_224(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('vit_standard_small_patch16_224', pretrained=pretrained,
                                       **dict(model_kwargs, **kwargs))
    return model


@register_model
def vit_standard_base_patch16_224(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16)
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('vit_standard_base_patch16_224', pretrained=pretrained,
                                       **dict(model_kwargs, **kwargs))
    return model
